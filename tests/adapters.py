...
def run_multihead_self_attention(
    ...
    inst=MultiHeadSelfAttention(d_model,num_heads,in_features.device,in_features.dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    inst=MultiHeadSelfAttentionRoPE(d_model,num_heads,theta,max_seq_len,device=in_features.device,dtype=in_features.dtype)
    qkv_weight=torch.cat([q_proj_weight,k_proj_weight,v_proj_weight],dim=0)
    inst.load_state_dict({
        "qkv_proj.W":qkv_weight,
        "output_proj.W":o_proj_weight
    })
    return inst(in_features,token_positions)
